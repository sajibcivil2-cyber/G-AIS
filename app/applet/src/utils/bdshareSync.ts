...
/**
 * Fill missing trading days with synthetic (estimated, NOT real) candles via the server's
 * gap-fill endpoint. This does not fetch real prices — see server.ts /api/dse/bdshare-live
 * for why (dsebd.org disallows automated access and doesn't expose historical data anyway).
 * Server-returned candles are treated as untrusted input regardless: each stock's merged
 * series is re-validated/repaired (see databaseStorage.validateAndRepairStock) before being
 * accepted, so a bad batch can never silently corrupt stored prices.
 */
export async function syncLiveBdShareData(stocks: DseStockData[]): Promise<BdShareSyncResult> {
  ...
  if (freshness.isUpToDate || stocks.length === 0) {
    return {
      ...
      missingDates: [],
      message: 'No missing trading days to fill.',
  ...
  if (!response.ok) {
    throw new Error(`Gap-fill server returned status ${response.status}`);
  }

  const data = await response.json();

  if (!data.success) {
    throw new Error(data.error || 'Failed to fill missing trading days.');
  }
